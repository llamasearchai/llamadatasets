    ...
    def split(self, dataset: Dataset) -> Dict[str, Dataset]:
        ...
        for example in examples:
            if self.label_column not in example:
                raise KeyError(
                    f"Label column '{self.label_column}' not found in example"
                )
...
class TimeSplitter(BaseSplitter):
    ...
    def split(self, dataset: Dataset) -> Dict[str, Dataset]:
        ...
        if not examples or self.timestamp_column not in examples[0]:
            raise KeyError(
                f"Timestamp column '{self.timestamp_column}' not found in dataset"
            )

        # Sort by timestamp
        examples.sort(
            key=lambda x: x[self.timestamp_column], reverse=not self.ascending
        )
    ...
    def split(self, dataset: Dataset) -> Dict[str, Dataset]:
        ...
        for example in examples:
            if self.group_column not in example:
                raise KeyError(
                    f"Group column '{self.group_column}' not found in example"
                )
