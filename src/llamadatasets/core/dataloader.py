...
class DataLoader:
    ...
    def _load_csv(self) -> Union[Dataset, StreamingDataset]:
        ...
        if self.streaming:
            ...
            return StreamingDataset(
                source_type="csv", source_path=self.source_path, **self.kwargs
            )
        else:
    ...
    def _load_json(self) -> Union[Dataset, StreamingDataset]:
        ...
        if self.streaming:
            # Return a streaming dataset
            return StreamingDataset(
                source_type="json", source_path=self.source_path, **self.kwargs
            )
        else:
    ...
    def _load_parquet(self) -> Union[Dataset, StreamingDataset]:
        ...
        if self.streaming:
            # Return a streaming dataset
            return StreamingDataset(
                source_type="parquet", source_path=self.source_path, **self.kwargs
            )
        else:
