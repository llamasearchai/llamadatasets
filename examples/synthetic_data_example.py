...
def main():
    ...
    for i in range(3):
        ...
        logger.info(
            f"  Example {i+1} ({example['category']}): {example['text'][:100]}..."
        )
    ...
    template_generator = TemplateTextGenerator(
        templates=templates, variables=variables, fields=fields, seed=123
    )
    ...
    for i in range(3):
        ...
        logger.info(
            f"  Example {i+1} (Priority: {example['priority']}): {example['text']}"
        )
        logger.info(
            f"    Customer ID: {example['customer_id']}, Response Time: {example['response_time_hours']} hours"
        )
    ...
    # Clean text
    text_cleaner = TextCleanerTransformer(columns="text", target_columns="cleaned_text")
    template_dataset = text_cleaner(template_dataset)

    # Tokenize
    tokenizer = TokenizerTransformer(columns="cleaned_text", target_columns="tokens")
    ...
    splitter = StratifiedSplitter(
        label_column="priority", train_size=0.7, val_size=0.15, test_size=0.15, seed=42
    )
    ...
    for split_name, split_dataset in splits.items():
        ...
        logger.info(
            f"  {split_name.capitalize()} split ({len(split_dataset)} examples): {priority_counts}"
        )
    ...
    logger.info(
        f"Combined dataset has {len(combined_dataset)} examples from multiple sources"
    )
