...
def create_large_csv(filename, num_rows=100000):
    ...
    for chunk_idx in range(0, num_rows, chunk_size):
        ...
        data = {
            ...
            'category': np.random.choice(['business', 'tech', 'health', 'entertainment'], chunk_size_actual),
            'timestamp': [
        ...
        logger.info(f"  Wrote chunk {chunk_idx//chunk_size + 1}/{(num_rows-1)//chunk_size + 1}")
...
def main():
    ...
    def process_example(example):
        # Add a derived field
        example['avg_value'] = (example['value1'] + example['value2'] + example['value3']) / 3
        return example
    ...
    for i, example in enumerate(head_dataset):
        logger.info(f"  Example {i+1}: id={example['id']}, category={example['category']}")
    ...
    logger.info(f"Converted first 1000 examples to regular dataset in {convert_time:.2f} seconds")
