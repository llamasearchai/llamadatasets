...
def create_sample_csv(filename):
    ...
    data = {
        ...
        "category": np.random.choice(
            ["business", "tech", "health", "entertainment"], 100
        ),
...
def main():
    ...
    loader = DataLoader.from_json(json_file, cache_config=cache_config)
    dataset = loader.load()
    ...
    logger.info(
        f"After text cleaning, example has cleaned_text: {dataset[0]['cleaned_text']}"
    )
    ...
    stop_words_remover = StopWordsRemoverTransformer(
        column="tokens", target_column="filtered_tokens"
    )
    dataset = stop_words_remover(dataset)
    logger.info(
        f"After stop words removal, example has filtered_tokens: {dataset[0]['filtered_tokens']}"
    )
    ...
    logger.info(
        f"After filtering for tech category, dataset has {len(tech_dataset)} examples"
    )
    ...
    logger.info(
        f"After mapping, example has token_count: {dataset_with_counts[0]['token_count']}"
    )
    ...
    # Random split
    splitter = RandomSplitter(train_size=0.7, val_size=0.15, test_size=0.15, seed=42)
